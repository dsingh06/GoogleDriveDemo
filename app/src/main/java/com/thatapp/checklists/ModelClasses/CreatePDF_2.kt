...
class CreatePDF_2(val questions: ArrayList<QuestionItem>, val context: Context, val filename: String, val additionalDetails: String) {
	...
	fun startPDFCreation() {
		...
		val mainPTable = PdfPTable(5)
		mainPTable.widthPercentage = 100f
		mainPTable.setWidths(floatArrayOf(0.4f, 2.6f, 0.3f, 0.3f, 0.3f))

		val cellData = PdfPCell(Phrase("CheckList App"))
		cellData.colspan = 2
		mainPTable.addCell(cellData)
...
//        table.addCell(PdfPCell(table2))
		...
		try {
			...
			val celll = PdfPCell(table)
			coverTable.addCell(celll)
			celll.border = PdfPCell.NO_BORDER
